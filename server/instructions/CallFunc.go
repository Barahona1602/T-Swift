...
func NewCallFunc(lin int, col int, id string, par []interface{}) CallFunc {
	...
	if len(p.Params) == len(funcSym.ListDec) {
		for i := 0; i < len(p.Params); i++ {
			...
			if val.Tipo == funcSym.ListDec[i].(ParamsDeclaration).Tipo {
				...
				ast.SetError("El tipo de parámetro es incorrecto", p.Lin, p.Col)
				return nil
			}
		}
	} else {
		ast.SetError("Faltan parámetros en la función", p.Lin, p.Col)
		return nil
	}
