		...
		if len(resultStruct.Valor.([]interface{})) == len(p.ListExp) {
			...
			for i := 0; i < len(resultStruct.Valor.([]interface{})); i++ {
				...
				if resultStruct.Valor.([]interface{})[i].(environment.StructType).Id == p.ListExp[i].(environment.StructContent).Id {
					...
					} else if resultStruct.Valor.([]interface{})[i].(environment.StructType).Tipo == environment.UNKNOWN {
						valor[resultStruct.Valor.([]interface{})[i].(environment.StructType).Id] = tempVal
						result = environment.Symbol{Lin: p.Lin, Col: p.Col, Id: tempVal.Id, Tipo: environment.STRUCT, Valor: tempVal.Valor, Mutable: true}
					} else {
						ast.SetError("El tipo de parámetro de la variable "+p.ListExp[i].(environment.StructContent).Id+" es incorrecto", p.Lin, p.Col)
					}
				}
			}
			result = environment.Symbol{Lin: p.Lin, Col: p.Col, Tipo: environment.STRUCT, Valor: valor}
		} else {
			ast.SetError("La cantidad de valores en el struct es incorrecta", p.Lin, p.Col)
		}
	} else {
		ast.SetError("No se ha encontrado el struct correspondiente", p.Lin, p.Col)
	}
	return result
}
